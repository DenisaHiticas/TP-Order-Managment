...
// clasa AbstractDAO defineste operatiile pentru accesarea tabelei : inserare, actualizare, stergere
//FindById, FindAll
public class AbstractDAO<T>{
    ...
    private String createSelectQuery(String field) {
        ...
        // ? o sa fie inlocuit cu date din aplicatie
    ...
    public T findById(int id) {
        ...
        PreparedStatement statement = null;
        ResultSet resultSet = null;
        //O interogare generică este construită folosind id  Valoarea "id" poate fi înlocuită
...
// Utilizând ResultSet-ul obținut în urma executării instrucțiunii, vom obține lista de obiecte.
// Doar primul element este de interes, deoarece baza de date ar trebui să conțină doar o singură înregistrare cu id-ul specificat
    ...
    private List<T> createObjects(ResultSet resultSet) {
        ...
        try {
            //For each result from the ResultSet
            while (resultSet.next()) {
                ctor.setAccessible(true);
                //Create a new instance of type T
                T instance = (T)ctor.newInstance();
                for (Field field : type.getDeclaredFields()) {
                    //Retrieve from the current result the value of the current field
                    String fieldName = field.getName();
                    Object value = resultSet.getObject(fieldName);
                    //Obtain the method for setting a value to the field
                    PropertyDescriptor propertyDescriptor = new PropertyDescriptor(fieldName, type);
                    Method method = propertyDescriptor.getWriteMethod();
                    //using the obtained method set the value to the field
